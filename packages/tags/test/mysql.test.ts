...
describe('mysql.test.ts', () => {
  ...
  it('bind tags', async () => {
    ...
    assert(tag.list[0].name === 'xxx' && tag.list[0].id);
  });
  ...
  it('unbind & listInstanTags', async () => {
    ...
    assert(list[0].name === 'test-unbind-1' && list[1].name === 'test-unbind-2' )
    ...
    assert(afterUnbind.list[0].name === 'test-unbind-2' && afterUnbind.list[1].name === 'test-unbind-4' )
    // unbind all
    await tagService.unbind({
      objectId: 123,
      tags: []
    ...
    assert(afterUnbindAll.list.length === 0 && afterUnbindAll.total === 0);
  });
});