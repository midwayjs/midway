import { IOptions, ICommandHooksCore, ICoreInstance, ICommands, IHooks } from './interface/commandHookCore';
...
export = class CommandHookCore implements ICommandHooksCore {
    ...
    */
    public async invoke(commandsArray: string[], allowEntryPoints?: boolean, options?: any) {
    ...
    private getCoreInstance(): ICoreInstance {
        const { provider, log, service, config, extensions, commands } = this.options;
        const serviceData: any = Utils.deepMerge(
            service || {},
            {
                provider: {
                    name: provider
    ...
    // 设置 provider
    private setProvider(providerName: string, providerInstance: IProviderInstance) {
    ...
    // 加载命令,支持子命令
    private loadCommands(instance, commandsMap: any, commands: any, parentCommandList?: string[]) {
        if (!commands) {
            return;
        }
        Object.keys(commands).map((command: string) => {
            ...
            if (currentRank > currentCommand.rank) {
                ...
                currentCommand.lifecycleEvents = this.loadLifecycle(command, commandInstance.lifecycleEvents, parentCommandList);
            ...
            if (commandInstance.commands) {
                this.loadCommands(instance, commandsMap[command].command, commandInstance.commands, (parentCommandList || []).concat(command));
            }
            // 合并 options
    ...
    private loadLifecycle(command: string, lifecycleEvents: string[] | undefined, parentCommandList?: string[]) {
        const allLifecycles: string[] = [];
        const parentCommand = parentCommandList && parentCommandList.length ? `${parentCommandList.join(':')}:` : '';
        if (lifecycleEvents) {
    ...
    private getCommand(commandsArray: string[], allowEntryPoints?: boolean): any {
        ...
        for (command of commandsArray) {
            ...
            Object.assign(this.coreInstance.processedInput.options, commandOptions.options);
        }
        if (!cmdObj) {
            this.error('commandNotFound', { command, commandPath });
        }
    ...
    private commandOptions(commandOptions): any {
        ...
        for (const option in commandOptions) {
            ...
            options[option] = this.options.options[optionInfo.shortcut] || this.options.options[option];
        }
        return {
            options,
            usage
