...
import { IRabbitMQApplication, ListenerOptions } from './interface';
...
export class RabbitMQServer extends QueueManager<amqp.Connection, amqp.Channel> implements IRabbitMQApplication {
  ...
  createConnection(url, socketOptions): Promise<any> {
    return amqp.connect(url, socketOptions);
  }

  async assertQueue(queue: string, options?): Promise<void> {
