import midwayCore, {
  ...
  type MidwayApplicationManager as MidwayApplicationManagerType,
  type MidwayDecoratorService as MidwayDecoratorServiceType,
} from '@midwayjs/core';
import {
  DATA_SOURCE_KEY,
  ENTITY_MANAGER_KEY,
...
import { MikroDataSourceManager } from './dataSourceManager.js';
import { EntityName, RequestContext } from '@mikro-orm/core';

const { MainApp, Configuration, Init, Inject } = midwayCore;
@Configuration({
  importConfigs: [
    {
      default: {
...
/**
 * Wires MikroORM v7 data sources into Midway lifecycle and injection hooks.
 */
export class MikroConfiguration implements ILifeCycle {
  @MainApp()
  app: IMidwayApplication;

  @Inject('midwayDecoratorService')
  decoratorService: MidwayDecoratorServiceType;

  @Inject('midwayApplicationManager')
  applicationManager: MidwayApplicationManagerType;
