...
console.log('\n🧪 Testing validation-class-validator ESM build...\n');
...
try {
  ...
  assert.ok(pkg.ClassValidatorService, 'ClassValidatorService 类应该导出');
  console.log('✅ ClassValidatorService 类存在');
  ...
  const validationService = pkg.default.validateServiceHandler({
    get: token => {
  ...
  const tests = [
    ...
    { name: 'isRequired', fn: () => schemaHelper.isRequired(TestDTO, 'test'), expectBoolean: true },
    { name: 'isOptional', fn: () => schemaHelper.isOptional(TestDTO, 'test'), expectBoolean: true },
  ...
  for (const test of tests) {
    try {
      ...
      if (!test.noReturn) {
        if (test.expectBoolean) {
          assert.strictEqual(typeof result, 'boolean', `${test.name} 应该返回 boolean`);
        } else {
