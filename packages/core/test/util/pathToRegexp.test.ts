...
describe('path-to-regexp', function () {
  ...
  describe('arguments', function () {
    it('should work without different call combinations', function () {
      PathToRegexpUtil.toRegexp('/test');
      PathToRegexpUtil.toRegexp('/test', []);
      PathToRegexpUtil.toRegexp('/test', undefined, {});

      PathToRegexpUtil.toRegexp(/^\/test/);
      PathToRegexpUtil.toRegexp(/^\/test/, []);
      PathToRegexpUtil.toRegexp(/^\/test/, undefined, {});

      PathToRegexpUtil.toRegexp(['/a', '/b']);
      PathToRegexpUtil.toRegexp(['/a', '/b'], []);
      PathToRegexpUtil.toRegexp(['/a', '/b'], undefined, {});
    });

    it('should accept an array of keys as the second argument', function () {
      const keys = [];
      const re = PathToRegexpUtil.toRegexp(TEST_PATH, keys, {end: false});
    ...
    it('should throw on non-capturing pattern', function () {
      expect(function () {
        PathToRegexpUtil.toRegexp('/:foo(?:\\d+(\\.\\d+)?)');
    ...
    it('should throw on nested groups', function () {
      expect(function () {
        PathToRegexpUtil.toRegexp('/{a{b:foo}}');
  ...
  describe('rules', function () {
    TESTS.forEach(function (test) {
      const [path, opts, tokens, matchCases, compileCases] = test as any;

      describe(util.inspect(path), function () {
        const keys = [];
        const re = PathToRegexpUtil.toRegexp(path, keys, opts);

        // Parsing and compiling is only supported with string input.
        if (typeof path === 'string') {
          it('should parse', function () {
            expect(PathToRegexpUtil.parse(path, opts)).toEqual(tokens);
          });

          describe('compile', function () {
            compileCases.forEach(function (io) {
              const [params, result, options] = io;
              const toPath = PathToRegexpUtil.compile(path, {
                ...opts,
                ...options,
              });
          ...
          function () {
            matchCases.forEach(function (io) {
              ...
              if (typeof path === 'string' && params !== undefined) {
                const match = PathToRegexpUtil.match(path, options);
  ...
  describe('compile errors', function () {
    it('should throw when a required param is undefined', function () {
      const toPath = PathToRegexpUtil.compile('/a/:b/c');
    ...
    it('should throw when it does not match the pattern', function () {
      const toPath = PathToRegexpUtil.compile('/:foo(\\d+)');
    ...
    it('should throw when expecting a repeated value', function () {
      const toPath = PathToRegexpUtil.compile('/:foo+');
