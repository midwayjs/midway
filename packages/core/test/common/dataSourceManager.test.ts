...
describe('test/common/dataSourceManager.test.ts', () => {
  ...
  class CustomDataSourceFactory extends DataSourceManager<any> {
    ...
    protected createDataSource(config, dataSourceName: PropertyKey): any {
  ...
  it('should test symbol dataSourceName', async () => {
    class EntityA {}

    const instance = new CustomDataSourceFactory();
    expect(instance.getName()).toEqual('test');

    const sym = Symbol.for('db2');
    ...
    expect(instance.getDataSource(sym).entitiesLength).toEqual(1);
    expect(instance.getDataSource('db2')).not.toBeDefined();

    expect(instance.getDataSourceNameByModel(EntityA)).toEqual(sym);
