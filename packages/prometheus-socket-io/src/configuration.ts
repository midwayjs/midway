...
export class AutoConfiguration {
  ...
  async onReady(contanier) {
    let dataService: DataService = await this.socketApp.getApplicationContext().getAsync(DataService);
    ...
    this.socketApp.on('connection', async socket => {
      ...
      socket.emit = (event: string, ...data: any[]) => {
          if (!blacklisted_events.has(event)) {
              ...
              dataService.inc('bytesTransmitted', labelsWithEvent, dataToBytes(data));
      ...
      (socket as any).onevent = (packet: any) => {
          if (packet && packet.data) {
              ...
              if (event === 'error') {
                  ...
                  let labelsWithEvent = { event: event, ...labels };
                  dataService.inc('bytesReceived', labelsWithEvent, dataToBytes(data));
