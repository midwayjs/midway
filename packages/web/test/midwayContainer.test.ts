...
import {MidwayContainer} from '../src/midwayContainer';
import {App} from './fixtures/enhance/ts-app-inject/app';
...
describe('/test/unit/midwayContainer.test.ts', () => {
  ...
  it('should scan app dir and inject automatic', () => {
    ...
    container.load({
      loadDir: path.join(__dirname, './fixtures/enhance/ts-app-inject')
    });
  ...
  it('should load js dir and inject with $', () => {
    const container = new MidwayContainer();
    container.load({
      loadDir: path.join(__dirname, './fixtures/enhance/js-app-inject')
    });
