...
## 通过 MIME 检测上传文件的类型，只允许负荷规则的文件上传

部分`恶意用户`，会尝试将 `.php` 等 WebShell 修改扩展名为 `.jpg`，来绕过基于扩展名的白名单过滤规则，在某些服务器环境内，这个 jpg 文件依然会被作为 PHP 脚本来执行，造成安全风险。

因此，`@midwayjs/upload` 组件提供了 `fileTypeWhiteList` 配置参数 **【请注意，此参数无默认值设置，即默认不校验】**，您可以通过此配置设置允许的文件 MIME 格式，规则为由数组 `[扩展名, mime, [...moreMime]]` 组成的 `二级数组`，例如：
...
您也可以使用 `@midwayjs/upload` 组件提供的 `uploadFileTypeList` 变量，作为默认的 MIME 校验规则，它提供了常用的 `.jpg`、`.png`、`.psd` 等文件扩展名的 MIME 数据：
...
但是，我们依然建议您在条件允许的情况下，设置 `fileTypeWhiteList` 参数，这将提升您的应用程序安全性。
