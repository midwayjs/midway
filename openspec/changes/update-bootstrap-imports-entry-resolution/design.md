...
- Decision: core 提供共享的 Bootstrap imports 解析函数；`@midwayjs/mock` 使用它显式组合额外测试组件、内部匿名 Configuration 和项目默认入口，保持 mock 的 additive imports 契约。
...
- 内部启动器若在调用 core 前注入自己的 imports，会意外关闭默认入口探测。仓库内调用方需要显式解析默认入口；其中 mock 的 `options.imports` 按既有文档继续表示额外测试组件。
