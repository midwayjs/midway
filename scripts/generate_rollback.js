...
for (const item of data) {
  if (item.private === false) {
    const remoteVersion = execSync(`npm show ${item.name} version`)
      .toString()
      .replace('\n', '');

    const localVersion = item.version;
    console.log(
      `----> ${item.name} local=${localVersion} remote=${remoteVersion}`
    );
...
writeFileSync(
  join(__dirname, `./rollback/rollback-${currentVersion}.sh`),
  arr.join('') + tnpmArr.join('') + diff.join('')
);
