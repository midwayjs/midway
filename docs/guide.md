...
* `app/router.ts` 可选，用于配置 URL 路由规则，具体参见 [Router](http://begg.alibaba-inc.com/zh-cn/basics/router.html)。
* `app/controller/**` 用于解析用户的输入，处理后返回相应的结果，具体参见 [Controller](http://begg.alibaba-inc.com/zh-cn/basics/controller.html)。
* `app/middleware/**` 可选，用于编写中间件，具体参见 [Middleware](http://begg.alibaba-inc.com/zh-cn/basics/middleware.html)。
* `app/public/**` 可选，用于放置静态资源，具体参见内置插件 [egg-static](https://github.com/eggjs/egg-static)。
* `app/extend/**` 可选，用于框架的扩展，具体参见[框架扩展](http://begg.alibaba-inc.com/zh-cn/basics/extend.html)。
* `config/config.{env}.ts` 用于编写配置文件，具体参见[配置](http://begg.alibaba-inc.com/zh-cn/basics/config.html)。
* `config/plugin.ts` 用于配置需要加载的插件，具体参见[插件](http://begg.alibaba-inc.com/zh-cn/basics/plugin.html)。
* `test/**` 用于单元测试，具体参见[单元测试](http://begg.alibaba-inc.com/zh-cn/core/unittest.html)。
* `app.ts` 和 `agent.ts` 用于自定义启动时的初始化工作，可选，具体参见[启动自定义](http://begg.alibaba-inc.com/zh-cn/basics/app-start.html)。关于`agent.js`的作用参见[Agent机制](http://begg.alibaba-inc.com/zh-cn/core/cluster-and-ipc.html#agent-%E6%9C%BA%E5%88%B6)。

由内置插件约定的目录：

* `app/public/**` 用于放置静态资源，可选，具体参见内置插件 [egg-static](https://github.com/eggjs/egg-static)。
* `app/schedule/**` 用于定时任务，可选，具体参见[定时任务](http://begg.alibaba-inc.com/zh-cn/basics/schedule.html)。

__若需自定义自己的目录规范，参见 __[Loader API](https://eggjs.org/zh-cn/advanced/loader.html)

* `app/view/**` 用于放置模板文件，可选，由模板插件约定，具体参见[模板渲染](http://begg.alibaba-inc.com/zh-cn/core/view.html)。
* `app/model/**` 用于放置领域模型，可选，由领域类相关插件约定，如 [egg-sequelize](https://github.com/eggjs/egg-sequelize)。

### 路由和控制器

midway 使用 koa-router 作为路由的承载着，同时在 ts 的语法上做了一些简化。
...
和以往的写法不同的是，__我们需要从容器中拿到对应的控制器实例，并绑定到路由的 pattern 上__。
...
对 Java 非常熟悉的同学对 IoC 会比较了解，Spring Framework 在整个业界使用的也是非常多，而对于动态语言 Javascript 来说，使用 IoC 是一种非常麻烦的事情，所以至今也没有非常成熟的体系。
...
我们会觉得奇怪，这些类或者属性并没有实例化，如何使用？这里就需要介绍 `IoC容器` 了。
...
Midway 内部使用了自动扫描的机制，在应用初始化之前，会扫描所有的文件，包含装饰器的文件会__自动绑定__到容器。
...
```typescript
applicationContext.getAsync('userService'); // 根据字符串 id 获取实例
applicationContext.getAsync(UserService);   // 传入类名，自动根据类目获取实例
```
...
* Request __默认__，请求作用域，生命周期随着请求链路，在请求链路上唯一，请求结束立即销毁
* Prototype 原型作用域，每次调用都会重复创建一个新的对象

在这三种作用域中，midway 的默认作用域为__请求作用域__，这也意味着，如果我们需要将一个对象定义为其他两种作用域，需要额外的配置。
